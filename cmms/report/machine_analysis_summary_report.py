...
class ReportMachineAnalysisSummary(models.AbstractModel): # Report File Name
    ...
    def _get_categories(self, _type):
        _categs = self._machines.filtered(lambda r: r.type_id == _type).mapped('category_id')
        return _categs
    ...
    def _get_machines(self, _type, _categ):
        ...
        for _mac in _machines:
            ...
            for _res in month_wise:
                _machine_entry[_res['invoice_date:month'].replace(year,'').strip()] = _res['amount']
