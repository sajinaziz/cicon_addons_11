...
class CiconDebtorsReportWizard(models.TransientModel):
    ...
    def create_excel_data(self,data):
        ...
        for _rec in records:
            ...
            worksheet.write(row, 4, _rec.get('total_chq', 0), format1)
            worksheet.write(row, 5, _rec.get('total_lc', 0), format1)
            worksheet.write_number(row, 6, _rec.get('TOTAL', 0), format1)
            worksheet.write_number(row, 7, _rec.get('30 Days', 0), format1)
            worksheet.write_number(row, 8, _rec.get('30-60 Days', 0), format1)
            worksheet.write_number(row, 9, _rec.get('60-90 Days', 0), format1)
            worksheet.write_number(row, 10, _rec.get('90-120 Days', 0), format1)
            worksheet.write_number(row, 11, _rec.get('120-150 Days', 0), format1)
            worksheet.write_number(row, 12, _rec.get('150-180 Days', 0), format1)
            worksheet.write_number(row, 13, _rec.get('6-12 Months', 0), format1)
            worksheet.write_number(row, 14, _rec.get('Above Year', 0), format1)
